...
int dbg_index = 0;
uint64_t temp_pml4e[512];
uint64_t temp_pdpe[512];
uint64_t temp_pde[512];
uint64_t temp_kern_pt[512];
uint64_t temp_vga_pt[512];


void setPds(uint64_t addr, uint64_t *pt) {
  temp_pml4e[(0x1ff & (addr)) >> 39] = (uint64_t)(temp_pdpe - kernmem_offset) ;
  temp_pdpe[(0x1ff & (addr)) >> 30] = (uint64_t)(temp_pde - kernmem_offset);
  temp_pde[(0x1ff & (addr)) >> 21] = (uint64_t)(pt - kernmem_offset);
}
...
void pt_initialise(uint32_t* modulep) {
  ...
  for (; kernStart <  kern_physfree; kernStart += BYTES_PER_PAGE) {
    ...
    int idx = (0x1ff & ( kernmem_offset + kern_physbase)) >> 12 ;
    temp_kern_pt[idx] = (kernStart);
  }
  ...
  for (i = 0; i < 80*25*4*2; i+= BYTES_PER_PAGE)
      ...
      */
  for (; i < 0x100000; i += BYTES_PER_PAGE) {
    int idx = (0x1ff & i) >> 12 ;
    temp_vga_pt[idx] = i;
  }
